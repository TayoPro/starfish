...
def addtocart(request):
    if request.method == 'POST':
        ...
        if cart:
            basket = ShopCart.objects.filter(user__username=request.user.username, product_id=item.id)
            if basket:
...
def placeorder(request):
    if request.method == 'POST':
        ...
        cburl = 'http://54.177.58.62/completed/'
        ...
        data = {'reference':pay_code,'email':user.email, 'amount': int(total), 
        'callback_url':cburl, 'order_number':cart_code}
